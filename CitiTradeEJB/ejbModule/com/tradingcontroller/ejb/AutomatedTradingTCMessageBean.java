...
@MessageDriven(activationConfig = {
		...
		@ActivationConfigProperty(propertyName = "destinationType", propertyValue = "javax.jms.Queue") }, mappedName = "ATQueue")
public class AutomatedTradingTCMessageBean implements MessageListener {
	...
	@EJB
	ITwoMovingAverages twomovingaverageTradingController;

	private static Logger LOGGER = Logger
			.getLogger(AutomatedTradingTCMessageBean.class.getName());
	...
	public void onMessage(Message message) {
		...
		try {
			if (message instanceof TextMessage) {
				...
				System.out.println("ATMessageBean Received:"
						+ textMessage.getText());
				// This object will have only the symbol and the amount
				TC_ATObject obj = TradingController.tradeFromXML(textMessage
						.getText());
				if (obj.getAlgo().equals(ALGO_BB)) {
					...
					autoTradingController
							.startNewTrade(obj.getSymbol(), obj.getLoss(),
									obj.getProfit(), obj.getAmtToTrade());
				} else if (obj.getAlgo().equals(ALGO_TMA)) {

					if (!twomovingaverageTradingController.isMonitoring()) {
						twomovingaverageTradingController.startMonitoring();
					}
					twomovingaverageTradingController
							.startNewTrade(obj.getSymbol(), obj.getLoss(),
									obj.getProfit(), obj.getAmtToTrade());
					// obtain a long moving average over some defined period of
					// a stock
					// (symbol, startYear, startMonth, startDay, endYear,
					// endMonth, endDay)

					/*
					 * ArrayList<HistoricalData> historicalDataTwoDays =
					 * marketDataHandler
					 * .getHistoricalDataBySymbol(obj.getSymbol(), 2015, 8, 26,
					 * 2015, 8, 26);
					 * 
